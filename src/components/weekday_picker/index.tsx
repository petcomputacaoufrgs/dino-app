    ...
    return (
        <div className='week_day_picker'>
            {weekState.weekDays.map((weekday, index) => (
                <div role="checkbox" aria-checked={weekday.isSelected} className='week_day_picker__weekday_circle' key={index}>
                    <button className={getCircleClass(weekday)} onClick={() => onWeekdayClick(weekday)}>
                        {weekday.name}
                    </button>
                </div>
            ))}
