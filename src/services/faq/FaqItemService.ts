...
import FaqItemRepository, { FaqItemRepositoryImpl } from '../../storage/database/faq/FaqItemRepository'
...
> {
 async convertModelToEntity(model: FaqItemDataModel): Promise<FaqItemEntity | undefined> {
  ...
  async convertEntityToModel(entity: FaqItemEntity): Promise<FaqItemDataModel | undefined> {
  ...
  public getFaqItemByFilter(faq: FaqEntity, faqItem: FaqItemEntity[], searchTerm: string): FaqItemEntity[] {
    return faqItem.filter((item) => item.localFaqId === faq.id && StringUtils.contains(item.question, searchTerm))
  }
} 
