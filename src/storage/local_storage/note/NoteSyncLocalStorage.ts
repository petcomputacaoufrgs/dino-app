...
class NoteSyncLocalStorage extends BaseLocalStorage {
  ...
  getShouldSyncOrder = (): boolean => {
    const should = this.get(LS_Constants.NOTE_ORDER_SHOULD_SYNC)
  ...
  setShouldSyncOrder = (should: boolean) => {
    this.set(LS_Constants.NOTE_ORDER_SHOULD_SYNC, JSON.stringify(should))
  }

  removeShouldSyncOrder = () => {
    this.remove(LS_Constants.NOTE_ORDER_SHOULD_SYNC)
  }
